...
#
# Env: NDSUI_IP (brick IP), NDSUI_PW (ssh password, stock default: root)
#
...
"$PLINK" -ssh root@$IP -pw "$PW" -batch "[ -f /usr/trimui/bin/runtrimui.sh.ndsui-backup ] && cp -f /usr/trimui/bin/runtrimui.sh.ndsui-backup /usr/trimui/bin/runtrimui.sh; mkdir -p /mnt/SDCARD/System/starts; true"
"$PSCP" -pw "$PW" "$STAGE_WIN\\boot\\ndsui_boot.sh" root@$IP:/mnt/SDCARD/System/starts/ndsui_boot.sh
"$PLINK" -ssh root@$IP -pw "$PW" -batch "chmod +x /mnt/SDCARD/System/starts/ndsui_boot.sh; grep -c NDSUI_HOOK /usr/trimui/bin/runtrimui.sh; ls /mnt/SDCARD/System/starts/"
